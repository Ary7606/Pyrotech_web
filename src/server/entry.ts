...
export function registerAdSenseTextRoutes(app: import('express').Express, config: AdSenseConfig) {
...
export function renderSsrDocument(
  ...
  options: { scriptHtml: string },
): string {
  const headContent = result.head + (options.scriptHtml ? '\n' + options.scriptHtml : '');
  return template
